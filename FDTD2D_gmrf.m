...
fmax = 0.4 * PHz;                    % maximum frequency
...
nx1 = 1;
nx2 = nx1 + round(gx/dx) - 1;

ny1 = 2 + NBUF(3) + NPML(3) + 1;
ny2 = ny1 + round(layer1/dy) - 1;

% ADD LAYER1 (GRATING)
ERxx(1,nx1:nx2,ny1:ny2) = er1;
ERyy(1,nx1:nx2,ny1:ny2) = er1;
ERzz(1,nx1:nx2,ny1:ny2) = er1;

ERxx(1,nx2+1:Nx,ny1:ny2) = er2;
ERyy(1,nx2+1:Nx,ny1:ny2) = er2;
ERzz(1,nx2+1:Nx,ny1:ny2) = er2;

WpE(1,nx1:nx2,ny1:ny2) = Wp_E1;

WpE(1,nx2+1:Nx,ny1:ny2) = Wp_E2;

URxx(1,nx1:nx2,ny1:ny2) = ur1;
URyy(1,nx1:nx2,ny1:ny2) = ur1;
URzz(1,nx1:nx2,ny1:ny2) = ur1;

URxx(1,nx2+1:Nx,ny1:ny2) = ur2;
URyy(1,nx2+1:Nx,ny1:ny2) = ur2;
URzz(1,nx2+1:Nx,ny1:ny2) = ur2;

WpM(1,nx1:nx2,ny1:ny2) = Wp_M1;

WpM(1,nx2+1:Nx,ny1:ny2) = Wp_M2;

for i=1:n_res
    
    GammaE(i,nx1:nx2,ny1:ny2) = Gamma_E1(i);
    W0E(i,nx1:nx2,ny1:ny2) = W0_E1(i);
    fE(i,nx1:nx2,ny1:ny2) = f_E1(i);

    GammaE(i,nx2+1:Nx,ny1:ny2) = Gamma_E2(i);
    W0E(i,nx2+1:Nx,ny1:ny2) = W0_E2(i);
    fE(i,nx2+1:Nx,ny1:ny2) = f_E2(i);

    GammaM(i,nx1:nx2,ny1:ny2) = Gamma_M1(i);
    W0M(i,nx1:nx2,ny1:ny2) = W0_M1(i);
    fM(i,nx1:nx2,ny1:ny2) = f_M1(i);
    
    GammaM(i,nx2+1:Nx,ny1:ny2) = Gamma_M2(i);
    W0M(i,nx2+1:Nx,ny1:ny2) = W0_M2(i);
    fM(i,nx2+1:Nx,ny1:ny2) = f_M2(i);

end

% ADD SUBSTRATE
ERxx(1,1:Nx,ny2+1:Ny) = er_sub;
ERyy(1,1:Nx,ny2+1:Ny) = er_sub;
ERzz(1,1:Nx,ny2+1:Ny) = er_sub;

URxx(1,1:Nx,ny2+1:Ny) = ur_sub;
URyy(1,1:Nx,ny2+1:Ny) = ur_sub;
URzz(1,1:Nx,ny2+1:Ny) = ur_sub;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
