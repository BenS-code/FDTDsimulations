...
% FREQUENCY RANGE
fmin = 0.2 * PHz;                    % minimum frequency
fmax = 0.6 * PHz;                    % maximum frequency
NFREQ = 100;                         % number of frequencies
FREQ = linspace(fmin,fmax,NFREQ);    % frequency array
        
% MATERIAL PROPERTIES
Wp_E = 1.14 * PHz;
Gamma_E = [0.15 0.05].* PHz;
W0_E = [1 0.83].* PHz;
f_E = [1 1.02];
...
update_plot = 100;           % update plot every "update_plot" steps
...
ERxx = erair * ones(1,Nx,Ny);
ERyy = erair * ones(1,Nx,Ny);
ERzz = erair * ones(1,Nx,Ny);
URxx = ones(1,Nx,Ny);
URyy = ones(1,Nx,Ny);
URzz = ones(1,Nx,Ny);
...
nx1 = 1;
nx2 = nx1 + round(slab_x/dx) - 1;

ny1 = 2 + NBUF(3) + NPML(3) + 1;
ny2 = ny1 + round(slab_y/dy) - 1;

% ADD THE SLAB
ERxx(1,nx1:nx2,ny1:ny2) = erslab;
ERyy(1,nx1:nx2,ny1:ny2) = erslab;
ERzz(1,nx1:nx2,ny1:ny2) = erslab;

WpE(1,nx1:nx2,ny1:ny2) = Wp_E;

URxx(1,nx1:nx2,ny1:ny2) = urslab;
URyy(1,nx1:nx2,ny1:ny2) = urslab;
URzz(1,nx1:nx2,ny1:ny2) = urslab;

WpM(1,nx1:nx2,ny1:ny2) = Wp_M;

for i=1:n_res
    
    GammaE(i,nx1:nx2,ny1:ny2) = Gamma_E(i);
    W0E(i,nx1:nx2,ny1:ny2) = W0_E(i);
    fE(i,nx1:nx2,ny1:ny2) = f_E(i);

    GammaM(i,nx1:nx2,ny1:ny2) = Gamma_M(i);
    W0M(i,nx1:nx2,ny1:ny2) = W0_M(i);
    fM(i,nx1:nx2,ny1:ny2) = f_M(i);

end

%% ADD SUBSTRATE
ERxx(1,nx1:nx2,ny2+1:Ny) = erslab;
ERyy(1,nx1:nx2,ny2+1:Ny) = erslab;
ERzz(1,nx1:nx2,ny2+1:Ny) = erslab;

WpE(1,nx1:nx2,ny1:ny2) = Wp_E;

URxx(1,nx1:nx2,ny2+1:Ny) = urslab;
URyy(1,nx1:nx2,ny2+1:Ny) = urslab;
URzz(1,nx1:nx2,ny2+1:Ny) = urslab;

WpM(1,nx1:nx2,ny2+1:Ny) = Wp_M;

for i=1:n_res
    
    GammaE(i,nx1:nx2,ny2+1:Ny) = Gamma_E(i);
    W0E(i,nx1:nx2,ny2+1:Ny) = W0_E(i);
    fE(i,nx1:nx2,ny2+1:Ny) = f_E(i);

    GammaM(i,nx1:nx2,ny2+1:Ny) = Gamma_M(i);
    W0M(i,nx1:nx2,ny2+1:Ny) = W0_M(i);
    fM(i,nx1:nx2,ny2+1:Ny) = f_M(i);
...
for T = 1 : STEPS
    ...
    if ~mod(T,update_plot)
        ...
        % SHOW FIELDS
        subplot(2,5,1);
        draw2D_BS(xa,ya,squeeze(ERzz),Ez_sqz,NPML,CMAP);
        axis equal tight;
        title([num2str(T) ' of ' num2str(STEPS)]);
        ylabel('TE mode');
        
        subplot(2,5,6);
        draw2D_BS(xa,ya,squeeze(ERxx),Ex_sqz,NPML,CMAP);
        axis equal tight;
        ylabel('TM mode');
